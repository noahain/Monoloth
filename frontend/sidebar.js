(function () {
    ...
    var ICONS = {
        ...
        'external-terminal': '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M15 6v12a3 3 0 1 0 3-3H6a3 3 0 1 0 3 3V6a3 3 0 1 0-3 3h12a3 3 0 1 0-3-3"/></svg>',
        ...
        tab: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="3" x2="9" y2="21"/></svg>',
        plus: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>'
    };

    var ICON_NAMES = ['folder', 'terminal', 'panel', 'external-terminal', 'copy', 'refresh', 'play', 'gear', 'search', 'git', 'download', 'upload', 'server', 'database', 'globe', 'package', 'wrench', 'zap', 'clock', 'trash', 'monitor', 'edit', 'tab'];

    // ---- Default Buttons ----
    var DEFAULT_BUTTONS = [
        { id: 'open_folder', name: 'Open Project Folder', icon: 'folder' },
        { id: 'open_cmd_project', name: 'Open CMD in Project', icon: 'external-terminal' },
        { id: 'open_cmd_panel', name: 'Open CMD Panel', icon: 'terminal' },
    ...
    function applyTabBarPosition() {
        ...
        var titlebarLeft = document.querySelector('.titlebar-left');
        ...
        var pos = _tabBarPosition;
        if (pos === 'titlebar' && !document.body.classList.contains('custom-titlebar-active')) {
            pos = 'standard';
        }

        document.body.classList.remove('tabbar-titlebar', 'tabbar-hidden');
        mainTabBar.classList.remove('in-titlebar');

        var spacer = mainTabBar.querySelector('.main-tab-spacer');
        if (spacer) spacer.remove();

        if (pos === 'titlebar') {
            document.body.classList.add('tabbar-titlebar');
            if (titlebarLeft && mainTabBar.parentElement !== titlebarLeft) {
                titlebarLeft.appendChild(mainTabBar);
            }
            mainTabBar.classList.add('in-titlebar');
            if (!mainTabBar.querySelector('.main-tab-spacer')) {
                var dragSpacer = document.createElement('div');
                dragSpacer.className = 'main-tab-spacer';
                dragSpacer.setAttribute('data-tauri-drag-region', '');
                mainTabBar.appendChild(dragSpacer);
            }
        } else {
            if (pos === 'hidden') document.body.classList.add('tabbar-hidden');
            if (termContainer && mainTabBar.parentElement !== termContainer && tabHost) {
                termContainer.insertBefore(mainTabBar, tabHost);
            }
        }

        if (window.MonolithTerminal && window.MonolithTerminal.updateTabBarVisibility) {
            window.MonolithTerminal.updateTabBarVisibility();
        }
    }
